...
st.markdown(
    ...
    /* TAG STYLES FOR SCANNER */
    .tag-buy { background-color: #28a745; color: white; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 0.8em; margin-left: 10px;}
    .tag-sell { background-color: #dc3545; color: white; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 0.8em; margin-left: 10px;}
    .tag-hold { background-color: #6c757d; color: white; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 0.8em; margin-left: 10px;}
...
def go_to_ticker(ticker):
    ...
    st.session_state["stock_query"] = ticker
...
def get_market_scanner_data():
    ...
    # Batch download data
    data = yf.download(
        tickers, period="6mo", interval="1d", group_by="ticker", threads=True
    )
    ...
    def get_verified_list(candidates, is_oversold):
        sorted_list = sorted(
            candidates, key=lambda x: x["RSI"], reverse=not is_oversold
        )
        verified = []
        for item in sorted_list:
            ...
            if is_oversold and item["RSI"] >= 30:
                continue
            if not is_oversold and item["RSI"] <= 70:
                continue
...
def analyze_chart_with_gemini_cached(
    ticker, _df_monthly_summary, _latest_indicators, api_key, model_name
):
    if not api_key:
        return None
...
def get_quick_analysis(ticker, api_key, model_name):
    try:
        ...
        return analyze_chart_with_gemini_cached(
            ticker, monthly_str, indicators_str, api_key, model_name
        )
    except:
        return None
...
if api_key:
    try:
        ...
        default_index = (
            opts.index(default_model_name) if default_model_name in opts else 0
        )
        if opts:
            selected_model = st.sidebar.selectbox(
                "Choose AI Model", opts, index=default_index
            )
    except:
        pass
...
page = st.sidebar.radio(
    "Go to",
    ["Global Headlines", "Market Scanner", "Stock Analyst Pro"],
    key="navigation",
)
...
if page == "Global Headlines":
    ...
    indices = [
        {"n": "S&P 500", "t": "^GSPC", "f": "SPY"},
        {"n": "Nasdaq", "t": "^IXIC", "f": "QQQ"},
        {"n": "Gold", "t": "GC=F", "f": "GLD"},
        {"n": "Oil", "t": "CL=F", "f": "USO"},
    ]
    ...
    for i, x in enumerate(indices):
        ...
        if h.empty:
            h = yf.Ticker(x["f"]).history(period="5d")
        with cols[i]:
            if len(h) >= 2:
                cur = h["Close"].iloc[-1]
    ...
    if not api_key:
        ...
        raw = fetch_rss_feed()
        for i in raw:
            st.write(f"- [{i['title']}]({i['link']})")
    else:
        with st.spinner("Analyzing news sentiment..."):
            items = fetch_rss_feed()
            ai_items = summarize_news_with_gemini(
                items, api_key, selected_model
            )
            ...
            for index, item in enumerate(ai_items):
                ...
                try:
                    dt = parsedate_to_datetime(item["pub_date"]).strftime(
                        " %H:%M"
                    )
                except:
                    dt = ""
                ...
                with st.expander(f"🕒 {dt} | {item['title']}"):
                    ...
                    with c_sig:
                        st.markdown(
                            f"<span style='color:{col}; font-weight:bold; font-size:1.1em;'>{sig}</span>",
                            unsafe_allow_html=True,
                        )
    ...
    if query:
        ...
        if len(res) > 0:
            exact_match = next(
                (item for item in res if item["symbol"] == query.upper()), None
            )
            if exact_match:
                selected_ticker = exact_match["symbol"]
                st.success(
                    f"Selected: {selected_ticker} - {exact_match['name']}"
                )
            else:
        ...
        if selected_ticker:
            ...
            with st.spinner(f"Analyzing {selected_ticker}..."):
                ...
                if info:
                    ...
                    with c2:
                        st.header(
                            f"{info.get('shortName')} ({selected_ticker})"
                        )
                    ...
                    with st.expander(
                        "📘 Reference: Chart Patterns, Signals & Success Rates"
                    ):
                        ...
                        c2.metric(
                            "Double Bottom", "88% Success", "Bullish Reversal"
                        )
                        c3.metric(
                            "Desc. Triangle", "87% Success", "Bearish Breakout"
                        )

                        st.markdown("---")
                        st.markdown("### 📊 Comprehensive Pattern Guide")

                        t_rev, t_con, t_tr = st.tabs(
                            ["🔄 Reversal", "➡️ Continuation", "📈 Trend"]
                        )
                        ...
                        with t_rev:
                            ...
                            with rev_cols[0]:
                                ...
                                st.markdown(
                                    ...
                                    | Pattern | Signal |
                                    | :--- | :--- |
                                    | **Inv. Head & Shoulders**{check("Head & Shoulders")} | **BUY** |
                        ...
                        with t_tr:
                            ...
                            st.markdown(
                                f"""
                                - **Ascending Staircase**{check("Staircase")}: Higher Highs & Higher Lows -> **BUY** dips.
                                - **Descending Staircase**{check("Staircase")}: Lower Highs & Lower Lows -> **SELL** rallies.
                                """
                            )
                    ...
                    tabs = st.tabs(
                        ["Chart", "Fundamentals", "Financials", "News"]
                    )
                    with tabs[0]:
                        fig = make_subplots(
                            rows=2,
                        ...
                        fig.add_trace(
                            go.Bar(
                                x=hist.index, y=hist["Volume"], name="Vol"
                            ),
                            row=2,
                            col=1,
                        )
                    ...
                    with tabs[3]:
                        st.subheader("📰 News Search & Filter")
                        search_term = st.text_input(
                            "Filter headlines by keyword:",
                            placeholder="e.g. Earnings, CEO, Analyst...",
                        )
                        ...
                        if news:
                            ...
                            if filtered_news:
                                ...
                                st.warning(
                                    f"No news found matching '{search_term}'."
                                )
                        else:
